...
def payment_status(request, username):
    if request.method == "POST":
        try:
            # Fetch order based on username
            order = Order_details.objects.filter(user__username=username, status="Pending").first()
            ...
            if order:
                ...
                return HttpResponse("No pending order found for this user.", status=404)
